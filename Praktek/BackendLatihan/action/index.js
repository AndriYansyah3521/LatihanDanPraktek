...
async function getData(request, reply) {
    ...
    const sql = "SELECT id, nama, alamat, kota, pendapatan FROM tabeldata where nama ilike '%'||$3||'%' AND alamat ilike '%'||$4||'%' order by "
        + orderby
        + ' '
    ...
    console.log(res);
    const sqlcount = `SELECT COUNT(id) FROM tabeldata where nama ilike '%'||$1||'%' AND alamat ilike '%'||$2||'%' `;
...
async function savedata(request, reply) {
    ...
    const sql = 'INSERT INTO public.tabeldata (nama, alamat, kota, pendapatan) VALUES ($1, $2, $3, $4 ) RETURNING id'
...
async function getDatabyId(request, reply) {
    ...
    let sql = 'SELECT id, nama, alamat, kota, pendapatan FROM tabeldata WHERE id = $1';
